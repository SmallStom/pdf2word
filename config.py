...
# 模板标准字号列表（磅值），用于字号映射
# 按固定格式规范：三号16 / 小三15 / 四号14 / 小四12
TEMPLATE_FONT_SIZES = [16, 15, 14, 12]
...
HEADING_PATTERNS = [
    ...
    (1, re.compile(r'^第\s*[零一二三四五六七八九十百千0-9]+\s*[章部分编篇]\b.*')),
    (1, re.compile(r'^Chapter\s*\d+\s*.+', re.IGNORECASE)),
    # 一级标题：附件一/附件1
    (1, re.compile(r'^附件\s*[一二三四五六七八九十0-9]+\s*[:：、]?\s*.+')),
    # 一级标题：附表1 / 附表 一
    (1, re.compile(r'^附表\s*[一二三四五六七八九十0-9]+\s*[:：]?\s*.+')),
    # 二级标题：第一节/第二节/第三节
    (2, re.compile(r'^第\s*[零一二三四五六七八九十百千0-9]+\s*节\b.*')),
    # 二级标题：第X条/款
    (2, re.compile(r'^第\s*[零一二三四五六七八九十百千0-9]+\s*[条款]\b.*')),
    # 二级标题：中文编号"一、二、三、"
    (2, re.compile(r'^[一二三四五六七八九十]+\s*[、]\s*.+')),
    # 三级标题："1. 招标条件" "2. 项目概况与招标范围" 等一级条款
    # 标题文字 <= 12 字符，避免把"1. 投标人须知全文"误判
    (3, re.compile(r'^\d+[\.\．]\s*[^0-9.\s].{0,11}$')),
    # 四级标题："1.1 总则" "3.1 资格要求" 等二级条款（标题文字 <= 15 字符）
    (4, re.compile(r'^\d+[\.\．]\d+[\.\．]?\s*[^0-9.\s].{0,14}$')),
    # 五级及以下："1.1.1 xxx" / "3.1.1 xxx" 视为正文列表项，不识别为 heading
]
...
BODY_FONT = {
    'cn': '宋体',
    'en': 'Times New Roman',
    'size_pt': 12,          # 小四号（固定格式规范）
    'bold': False,
    'line_spacing': 1.5,    # 固定格式规范：1.5 倍行距
    'space_before_pt': 0,
    'space_after_pt': 0,
